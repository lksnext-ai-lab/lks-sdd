...
If the project has .lks-sdd/distribution-lock.json, read it and run its exact runtime/scripts/lks_sdd.py runtime-doctor with the project path. Stop on invalid integrity; never substitute the newer installed core. Read .github/lks-sdd-host.md, then runtime/skills/lks-sdd-help/SKILL.md and its references from that pinned runtime. Resolve <plugin-root> in that workflow to the pinned runtime.
