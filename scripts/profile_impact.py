"""Conservative read-only report of affected structural profiles."""
...
def impact(paths: list[str]) -> dict:
    ...
    for entry in catalog.get("profiles", []):
        ...
        for path in paths:
            ...
            elif path in {"profiles/catalog.json", "profiles/architecture-contracts.json"}:
                reasons.append("shared structural contract changed: " + path)
        ...
        if reasons:
            affected.append({"profile_id": entry["id"], "contract_id": bundle.driver.get("variant", {}).get("contract_id"),
                             ...
                             "structural_lock": entry["path"] + "/technology-profile.lock.json",
                             "fixtures": [s["from"] for s in bundle.driver.get("prepare", {}).get("sources", [])],
                             "reasons": sorted(set(reasons))})
    return {"schema_version": "1.0", "read_only": True,
