...
4. Para esta fuente local no Git, conservar la carpeta de la versión anterior y registrar su ruta para rollback; ejecutar `codex plugin marketplace remove lks-sdd-development` y `codex plugin marketplace add "RAÍZ_2.1.2_VERIFICADA"`. No usar `upgrade` para esta fuente. Para un alta inicial basta `add`; completar la activación en la superficie de Codex disponible.
...
El bundle 2.1.2 continúa siendo `skills-only` y no instala Atlassian Rovo ni configura Microsoft Entra. Para usar `jira-hybrid` o `milestone-reporting`, el participante debe disponer separadamente del peer Rovo, de una conexión Jira válida y de permisos suficientes. La aprobación stable no convierte esas interoperabilidades en `passed`. Sin Rovo, `repository-only` sigue completo; las declaraciones tecnológicas de los proyectos continúan siendo decisiones locales, sin soporte global implícito.
