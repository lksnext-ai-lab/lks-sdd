"""Single-delivery implementation using only prompt-compact_complete.md."""
...
def ledger_total(amounts):
    ...
    # Extra precision avoids rounding the sum before the prescribed cent rounding.
    precision = max(
        28,
...
def fold_events(events):
    ...
    for event in events:
        ...
        if group not in winners or event["seq"] > winners[group]["seq"]:
            winners[group] = event
    # The prompt does not specify the output container/order: return visible
    # event dictionaries in deterministic (tenant, key) order.
    return [deepcopy(winners[group]) for group in sorted(winners)
            if not winners[group].get("deleted", False)]


def can_read(user, resource):
    if (user.get("disabled", False)
            or user["tenant"] != resource["tenant"]
            or user["id"] in resource.get("denied_user_ids", [])):
        return False
    return bool(
        "admin" in user.get("roles", [])
        or resource.get("owner_id") == user["id"]
        or resource.get("public", False)
    )
...
def retry_delay(value, now, cap):
    ...
    if re.fullmatch(r"[0-9]+", value):
        # Leading zeroes do not turn a valid integer into an int-limit error.
        digits = value.lstrip("0") or "0"
        cap_text = str(cap)
        if len(digits) > len(cap_text):
            return cap
        return min(int(digits), cap)
    # HTTP-date timestamps must explicitly use GMT rather than local time.
...
def migration_order(steps, applied):
    ...
    pending = {step_id for step_id in by_id if step_id not in applied_ids}
    predecessors = {}
    successors = {step_id: set() for step_id in pending}
    for step_id in pending:
        step = by_id[step_id]
        # The field spelling is unspecified; accept deps and depends_on.
        dependencies = set(step.get("deps", step.get("depends_on", [])))
        missing = dependencies - set(by_id) - applied_ids
        if missing:
