...
La entrada del marketplace usa la forma estándar `local`, ruta `./plugins/lks-sdd`, instalación `AVAILABLE`, autenticación `ON_INSTALL` y categoría `Developer Tools`. El builder añade un manifiesto por archivo y `SHA256SUMS`, rechaza enlaces, salidas dentro del repositorio y patrones de secretos conocidos.
