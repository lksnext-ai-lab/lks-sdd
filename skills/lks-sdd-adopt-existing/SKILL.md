...
2. Ejecuta `scripts/inspect_repository.py`. No combines la inspección con ningún comando de aplicación, build, test, contenedor, migración, hook, gestor de paquetes o red.
...
5. Ejecuta `scripts/validate_adoption.py`. Si la baseline está `stale`, repite el inventario y la reconciliación afectada.
6. Ejecuta `scripts/materialize_adoption.py --dry-run`, presenta rutas, colisiones y `preview_hash`, y espera autorización explícita.
7. Solo entonces ejecuta `--apply --authorize --preview-hash <hash>`.
