...
La versión `0.11.0` conserva M0–M5 y las seis skills: ayuda, definición, adopción, readiness, implementación y verificación. Implementa como candidate el contrato propuesto 1.5: la experiencia profesional puede ser completamente local (`repository-only`) o usar Jira como proyección operativa (`jira-hybrid`) mediante Atlassian Rovo. En Jira se elige además `projection-only` o `milestone-reporting`, con coordinación `advisory` o `required-before-execution`. El reporting publica solo hitos significativos ya durables, con comentario saneado y transición opcional basada en IDs confirmados; una confirmación autoriza el preview completo y cada operación conserva su propio recibo. Los Markdown del repositorio siguen siendo la única autoridad semántica y no existe Jira-only. Los proyectos nuevos usan `method_version: 1.5.0` y `schema_version: 1.5`; 1.0–1.4 permanecen validables y solo cambian mediante migraciones explícitas de un salto. Los canales humanos, la interoperabilidad real Rovo/Jira y el piloto siguen `not-run`; M6 y la promoción a `stable` permanecen pendientes. El bundle continúa sin MCP, cliente Jira, conectores propios, hooks, apps ni agentes ejecutables.
...
- `schemas/`: contratos 1.0/1.1/1.2/1.3/1.4/1.5 del índice, front matter, documentos, tracking/reporting, perfiles, drivers, locks y certificaciones.
...
- `specs/proposed/`: extensiones candidate 1.4 de tracking y 1.5 de reporting Jira, todavía no canónicas ni aprobadas como política corporativa.
...
- `docs/ARCHITECTURE.md`: arquitectura de la versión 0.11.0, M0–M5 y límites aún vigentes.
