...
La decisión y suficiencia funcional pertenecen a `specification_readiness`; el plan, release y tareas a `delivery_readiness`; la certificación de cada binding a `automation_support`. Una alternativa confirmada puede quedar bien especificada y a la vez mostrar `automation_support: unsupported`. Esa combinación bloquea la automatización, no autoriza cambiar de pila ni convierte la alternativa en una especificación deficiente.
