...
Los proyectos nuevos materializados por 0.10.0 usan método 1.4.0 y esquema 1.4. Los proyectos 1.0/1.1/1.2/1.3 siguen en compatibilidad y se migran mediante saltos explícitos. El renderer histórico 1.2 → 1.3 conserva método 1.3.0/plugin 0.9.1 y añade cobertura `PCH`, autorizaciones `AUTH`, ejecuciones `EXEC` y checkpoints `CKPT`; el salto 1.3 → 1.4 añade tracking pendiente sin inferir confirmaciones, personas, avance, evidencia o escrituras Jira.
