...
Esta skill no crea, edita, enlaza, comenta, asigna ni transiciona Jira. Ante drift, devuelve el menor paso de lectura y `reconcile-result` o `reconcile-event` necesario, sin ejecutarlo. No presenta la reconciliación como permiso para abandonar/cambiar un binding durable: 0.14.1 no ofrece `detach`/`rebind`.
