...
Las inicializaciones nuevas de LKS-SDD 0.10.0 usan `method_version: 1.4.0` y `schema_version: 1.4`. Los documentos 1.0, 1.1, 1.2 y 1.3 siguen siendo validables en compatibilidad; no se actualizan al instalar el plugin. Las migraciones son explícitas, reversibles y de un solo salto (`1.0 → 1.1 → 1.2 → 1.3 → 1.4`). El salto histórico 1.2 → 1.3 conserva método 1.3.0/plugin 0.9.1, mantiene el cuerpo humano, añade cobertura y definición ejecutable como pendientes e inicializa autorizaciones y ejecuciones vacías. El salto 1.3 → 1.4 añade tracking pendiente sin consultar Jira, crear issues ni inventar una elección humana.
