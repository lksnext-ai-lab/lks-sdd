...
Una materialización nueva con LKS-SDD 0.8.0 crea método 1.2.0 y esquema 1.2 con los 20 artefactos obligatorios. El inventario observado puede proponer fronteras arquitectónicas, pero no confirma `UNIT-###`, `BIND-###`, modelo de entrega, Git, entornos, release ni tareas. `ART-GOVERNANCE`, `ART-PLANS` y `ART-TASKS` nacen como propuesta/pendiente hasta una decisión humana. La revisión Git observada queda en la evidencia de adopción; `last_verified_revision` permanece `null` porque inspección estática no es verificación.
