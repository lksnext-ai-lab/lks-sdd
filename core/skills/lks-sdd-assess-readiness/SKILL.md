...
7. For the selected TASK slice, report `integration_applicability` from confirmed canonical `INT-###` rows. A backend-only or standalone frontend slice is deterministically `not-applicable`; a joint owner must select every unit/binding and an exact certified system composition. Component profile support cannot make a cross-unit slice ready. If the exact composition is absent, stale or uncertified, report `automation_support=unsupported` with the interface and next certification/reconciliation step.
