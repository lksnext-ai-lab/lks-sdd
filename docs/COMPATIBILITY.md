...
|---|---|---|
| Nuevo con LKS-SDD 0.14.2 | Método candidate `1.5.0`, esquema `1.5` | Conserva reporting Jira opcional, verificación incremental task-aware y perfiles OIDC simulados no productivos sin cambiar el schema del proyecto. |
...
La evidencia de verificación EVID mantiene `schema_version: 1.2`. En la forma canónica 0.14.2, una selección de un binding declara arriba su perfil y versión exactos; una selección multiperfil omite ese resumen y conserva todos los valores en bindings, locks y material de build. El lector compatible no reescribe evidencias 1.2 anteriores: acepta campos superiores ausentes o `null` únicamente cuando la identidad se deriva de forma única y coherente del resto del documento. Las contradicciones, conjuntos parciales y un `build_id` que no corresponda al material siguen siendo inválidos.
...
`active` expresa intención de producto; `supported` exige además una certificación exacta válida. `candidate` significa que el perfil puede documentarse y evaluarse, pero el plugin no garantiza todavía su recorrido reproducible `readiness → prepare → implement → verify`. `automation_coverage` puede mostrar preparación o gates definidos sin alterar ese estado. Un proyecto con React/Vite estático no debe forzarse al perfil completo con API, identidad y base de datos; debe seleccionar el perfil coherente con su arquitectura y quedar bloqueado si esa composición no está certificada.

0.12 añade `API-FASTAPI-ENTRA-PG-OCI` y `WEB-REACT-VITE-ENTRA-STATIC` como candidates exactos. Deben ligarse a unidades desplegables separadas cuando frontend y backend tengan fronteras distintas. Keycloak no es fallback de Microsoft Entra y las capabilities granulares no permiten fabricar un perfil de sistema dinámico.

0.13 añade `API-FASTAPI-SIMULATED-OIDC-PG-OCI` y `WEB-REACT-VITE-SIMULATED-OIDC-STATIC` como perfiles active certificados para desarrollo, CI, integración y aceptación-preproducción. Mantienen bindings independientes, `external_interoperability: not-applicable` y bloqueo productivo fail-closed; no acreditan ni sustituyen Microsoft Entra.
...
python "<plugin-root>/scripts/lks_sdd.py" traceability "<project-root>" --increment INC-001 --task TASK-001 --phase preimplementation --json
...
| Capacidad | Estado 0.14.2 candidate |
|---|---|
...
La candidate 0.14.2 no ofrece `detach` ni `rebind`: cuando existen mappings o recibos durables, no permite abandonar Jira ni cambiar site, proyecto o tipo. Reconciliar `uncertain`, `conflict` o una key modificada dentro del prefijo confirmado no elimina esa restricción. Un cambio de prefijo por rename o movimiento queda fuera de soporte y mantiene conflicto o reconciliación pendiente. Las proyecciones `confidential`/`restricted`, con secretos o datos personales detectables se bloquean; site y URL deben ser HTTPS sin credenciales, query ni fragmento. Véase `docs/JIRA-ROVO-INTEGRATION.md`.
