...
4. For contextual help, optionally run `python scripts/context_help.py <project-root>`. It validates the index and canonical Markdown in read-only mode, then separates structural validity from semantic definition coverage. When `ART-STATUS` contains the qualitative coverage table, show the compact, accessible groups `✓ Suficiente para avanzar`, `△ Requiere profundización`, `○ Aún desconocido`, `⛔ Bloqueos` and `→ Siguiente decisión`; the symbol helps scanning but the text carries the meaning. Offer per-dimension detail on request and never calculate a percentage. For legacy projects without the table, state that sufficiency cannot be inferred and use the conservative fallback instead of presenting structural validity as completeness.
