...
La versión `0.12.0` conserva M0–M5, las seis skills y el contrato de proyectos `method_version: 1.5.0` / `schema_version: 1.5`. Añade cuatro capabilities de identidad reutilizables y dos perfiles exactos Microsoft Entra candidate: `API-FASTAPI-ENTRA-PG-OCI` y `WEB-REACT-VITE-ENTRA-STATIC`. El frontend y el backend siguen siendo unidades/bindings independientes; no se abre composición dinámica ni se sustituye Entra por Keycloak. `automation_coverage` explica por binding el encaje, la preparación, los gates locales, la interoperabilidad externa y la evidencia pendiente, mientras `automation_support` continúa siendo estricto: los candidates siguen `unsupported`. La experiencia local y Jira mediante Atlassian Rovo de 0.11 se conservan, los Markdown siguen siendo la única autoridad semántica y 1.0–1.5 permanecen compatibles sin migración por esta actualización. La interoperabilidad real Entra y Rovo/Jira, los canales humanos y el piloto siguen `not-run`; M6 y la promoción a `stable` permanecen pendientes. El bundle continúa sin MCP, cliente Jira, conectores propios, hooks, apps ni agentes ejecutables.
...
- `docs/ARCHITECTURE.md`: arquitectura de la versión 0.12.0, M0–M5 y límites aún vigentes.
