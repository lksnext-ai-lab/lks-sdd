...
2. Read the [verification contract](references/verification-contract.md). Run `python scripts/run_verification.py <project-root> --increment <INC-###> --plan --json` and explain applicable checks, side effects, prerequisites, and omissions. A 0.6+ increment with an applicable interface always includes `visual-browser-review`.
3. Execute only after explicit authorization with `--execute --authorize`. Add `--containers` only when starting the local PostgreSQL/Keycloak stack is authorized and safe. For an applicable interface, provide reviewed browser evidence 1.1 with `--visual-evidence <local-json>`; bind it to the completed implementation, current UX/VIS baseline, viewports and screenshots as defined in the contract. Without it, the visual check remains `not-run` and the increment cannot be `verified`. Do not substitute a different command because a required tool is missing.
