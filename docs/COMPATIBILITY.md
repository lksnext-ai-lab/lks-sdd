...
|---|---|---|
| Nuevo con LKS-SDD 0.10.0 | Método candidate `1.4.0`, esquema `1.4` | Añade elección `repository-only`/`jira-hybrid` y tracking separado, conservando Markdown como autoridad. |
...
| Existente 1.0 | Compatible con reglas legacy y avisos | Ruta explícita `1.0 → 1.1 → 1.2 → 1.3 → 1.4`, una operación por salto. |
| Existente 0.9 | Sin salto directo | Ruta histórica `0.9 → 1.0`, un salto autorizado cada vez. |

La actualización del plugin no migra proyectos consumidores. Cada aplicación de migración exige preview, hash coincidente, backup externo, autorización expresa y validación posterior. `1.0 → 1.1` mantiene el bloqueo por `human_review_required`; `1.1 → 1.2` materializa gobierno, arquitectura, planes, tareas y bindings pendientes; `1.2 → 1.3` añade cobertura, huellas, autorizaciones y ejecuciones vacías; `1.3 → 1.4` añade el contrato de tracking sin crear issues ni inventar una decisión humana. Este último salto rechaza cualquier `EXEC-###` no terminal y conserva byte a byte los checkpoints históricos 1.3; los checkpoints creados después usan 1.4. El renderer histórico 1.2 → 1.3 conserva schema 1.3, método 1.3.0 y plugin 0.9.1.
...
| Capacidad | Estado 0.10.0 candidate |
|---|---|
...
| Transiciones, comentarios y adjuntos | No se presumen soportados; fail-closed si no existe operación exacta |
...
La candidate 0.10.0 no ofrece `detach` ni `rebind`: cuando existen mappings o recibos durables, no permite abandonar Jira ni cambiar site, proyecto o tipo. Reconciliar `uncertain`, `conflict` o una key modificada dentro del prefijo confirmado no elimina esa restricción. Un cambio de prefijo por rename o movimiento queda fuera de soporte y mantiene conflicto o reconciliación pendiente. Las proyecciones `confidential`/`restricted`, con secretos o datos personales detectables se bloquean; site y URL deben ser HTTPS sin credenciales, query ni fragmento. Véase `docs/JIRA-ROVO-INTEGRATION.md`.
