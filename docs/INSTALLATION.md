...
Descarga los assets de [v2.0.2](https://github.com/lksnext-ai-lab/lks-sdd/releases/tag/v2.0.2)
...
La distribución 2.0.2 tiene un núcleo y dos destinos. Para Copilot se recomienda
el **plugin de agente** `lks-sdd-copilot-plugin-v2.0.2.zip` y el procedimiento
...
3. El catálogo del repositorio ofrece **lks-sdd**, versión **2.0.2**. Selecciónalo y
...
El catálogo `.github/plugin/marketplace.json` apunta a `copilot-v2.0.2`, etiqueta
...
1. Verifica el SHA-256 y extrae `lks-sdd-copilot-plugin-v2.0.2.zip` en una
   carpeta corta y estable, por ejemplo `C:\LksPilot`. Debe quedar
   `C:\LksPilot\lks-sdd\plugin.json` junto a `skills`, `core` y `setup`.
   El paquete 2.0.2 resuelve internamente rutas largas de Windows y mantiene las
...
Esta alternativa usa el ZIP `lks-sdd-setup-v2.0.2.zip`, no el setup incluido en
...
Alternativamente, el ZIP `lks-sdd-marketplace-v2.0.2.zip` se puede extraer en esa
